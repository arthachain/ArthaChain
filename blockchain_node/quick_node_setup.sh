...
echo -e "\n🏥 Local Health Check:"
curl -s http://localhost:8080/api/health | jq . || echo "❌ Local API not responding"

echo -e "\n📈 Local Node Status:"
curl -s http://localhost:8080/api/status | jq . || echo "❌ Local status not available"

echo -e "\n🌐 Network Validators (from bootstrap):"
curl -s https://api.arthachain.in/api/validators | jq '.total_count' || echo "❌ Network validators not available"

echo -e "\n📦 Network Latest Block:"
curl -s https://api.arthachain.in/api/blocks/latest | jq '.height' || echo "❌ Network block info not available"
...
echo "  Local: curl http://localhost:8080/api/health"
echo "  Network: curl https://api.arthachain.in/api/validators"
echo ""
