...
impl TransactionGenerator {
    ...
    fn generate_batch(&mut self, batch_size: usize) -> Vec<Transaction> {
        ...
        for _ in 0..batch_size {
            ...
            } else if tx_type_roll < self.tx_mix.payment_percent + self.tx_mix.contract_deploy_percent {
                self.generate_contract_deploy()
            } else if tx_type_roll < self.tx_mix.payment_percent + self.tx_mix.contract_deploy_percent + self.tx_mix.contract_call_percent {
    ...
    fn generate_contract_call(&self) -> Transaction {
        ...
        let size = rng.gen_range(
            self.tx_mix.contract_call_min_bytes..self.tx_mix.contract_call_max_bytes
        );
        let mut data = vec![0u8; size];
        rng.fill(&mut data[..]);
    ...
    fn generate_data_storage(&self) -> Transaction {
        ...
        let size = rng.gen_range(
            self.tx_mix.data_storage_min_bytes..self.tx_mix.data_storage_max_bytes
        );
        let mut data = vec![0u8; size];
        rng.fill(&mut data[..]);
...
async fn run_real_world_simulation(config: &SimulationConfig) -> Result<()> {
    ...
    println!("Total data size processed: {:.2} MB", total_size_bytes as f64 / (1024.0 * 1024.0));
    println!("Average batch processing time: {:.2?}", avg_batch_time);
    
    // Print transaction type statistics
    println!("\nTransaction Type Statistics:");
    println!("----------------------------");
    for tx_type in &[TransactionType::Transfer, TransactionType::ContractDeploy, 
                    TransactionType::ContractCall, TransactionType::DataStorage] {
        ...
        println!("{:?}: {} transactions ({:.1}%) at {:.2} TPS", 
