...
async fn simulate_transactions(config: &SimulationConfig) -> Result<()> {
    ...
    println!("  - {:.1}% cross-shard transactions", config.cross_shard_ratio * 100.0);
    ...
    let transactions = generate_transactions(config.num_transactions, config.num_shards, config.cross_shard_ratio);
...
fn generate_transactions(count: usize, num_shards: usize, cross_shard_ratio: f32) -> Vec<Transaction> {
