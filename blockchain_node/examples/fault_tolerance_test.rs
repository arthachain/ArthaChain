...
impl Node {
    ...
    async fn process_batch(&mut self, batch: Vec<Transaction>) -> Result<usize> {
        ...
        let result = if self.resource_exhausted {
            ...
            self.execution_manager.process_transactions(half_batch).await
        } else {
...
async fn run_fault_tolerance_test(config: &FaultToleranceConfig) -> Result<()> {
    ...
    println!("  - {:.1}% failure probability", config.failure_probability * 100.0);
    println!("  - {:.1}% network delay probability", config.network_delay_probability * 100.0);
    ...
    while Instant::now() < end_time {
        ...
        for node in nodes.iter_mut() {
            if (node.failed || node.partitioned || node.resource_exhausted) && 
               rng.gen::<f64>() > 0.5 { // 50% chance to recover on each iteration
                node.recover();
                recoveries += 1;
            }
        }
        ...
        if now - last_report_time >= report_interval {
            ...
            println!("  Nodes: {}/{} healthy", 
                     nodes.iter().filter(|n| !n.failed && !n.partitioned && !n.resource_exhausted).count(),
                     nodes.len());
            ...
            println!("\n  Node Status:");
            for node in &nodes {
                println!("    Node {}: {} - {} txs processed, {} failures recovered, {:.2} TPS", 
                         node.id, 
                         node.status(), 
    ...
    let healthy_nodes = nodes.iter().filter(|n| !n.failed && !n.partitioned && !n.resource_exhausted).count();
    ...
    let healthy_node_tps: Vec<f64> = nodes.iter()
    ...
    if overall_tps > success_threshold * avg_healthy_tps * healthy_nodes as f64 / config.num_nodes as f64 && 
