                            ...
                            if let Some(events) = client_msg.events {
                                ...
                                for event_type in events {
                                    match event_type.as_str() {
                                        "new_block" => {
                                            if block_rx.is_none() {
                                                block_rx = Some(BroadcastStream::new(event_manager.block_tx.subscribe()));
                                                subscribed_events.push("new_block".to_string());
                                            }
                                        },
                                        "new_transaction" => {
                                            if transaction_rx.is_none() {
                                                transaction_rx = Some(BroadcastStream::new(event_manager.transaction_tx.subscribe()));
                                                subscribed_events.push("new_transaction".to_string());
                                            }
                                        },
                                        "consensus_update" => {
                                            if consensus_rx.is_none() {
                                                consensus_rx = Some(BroadcastStream::new(event_manager.consensus_tx.subscribe()));
                                                subscribed_events.push("consensus_update".to_string());
                                            }
                                        },
                                        _ => {}
                                    }
