...
use pyo3::prelude::*;
use pyo3::types::{PyDict, PyTuple, PyAnyMethods};
...
impl NeuralBase {
    ...
    pub async fn new(config: NeuralConfig) -> Result<Self> {
        Python::with_gil(|py| {
            ...
            let module = py.import("adaptive_network")?;
            let model_class = module.getattr("AdaptiveNetwork")?;
            
            // Extract the configuration values 
            let input_dim = if !config.layers.is_empty() { config.layers[0].input_dim } else { 10 };
            let output_dim = if !config.layers.is_empty() { config.layers.last().unwrap().output_dim } else { 10 };
            ...
            // Create the model instance with proper PyO3 0.24 call pattern
    ...
    pub async fn train(&mut self, batch_size: usize) -> Result<TrainingMetrics> {
        ...
        let experiences = match self.memory_buffer.sample_batch(batch_size) {
            Ok(exp) => exp,
            Err(_) => return Ok(TrainingMetrics {
                loss: 0.0,
                accuracy: 0.0,
                iterations: 0,
            }),
        ...
        // Train model
        Python::with_gil(|py| {
            // Convert to numpy arrays using proper PyO3 0.24 patterns
            let states_array = PyArray2::from_vec2(py, &states)?;
            let actions_array = PyArray1::from_slice(py, &actions);
            let rewards_array = PyArray1::from_slice(py, &rewards);
            let next_states_array = PyArray2::from_vec2(py, &next_states)?;
            let dones_array = PyArray1::from_slice(py, &dones);
            
            // Get model using proper PyO3 0.24 pattern
            ...
            let code = std::ffi::CString::new("loss = float(model.train_step(states, actions, rewards, next_states, dones))").unwrap();
    ...
    pub fn add_experience(&mut self, state: Vec<f32>, action: Vec<f32>, reward: f32, next_state: Vec<f32>, done: bool) {
    ...
    pub async fn predict(&self, input: &[f32]) -> Result<Vec<f32>> {
        ...
        Python::with_gil(|py| {
            ...
            let x = PyArray2::from_vec2(py, &input_vec)?;
    ...
    pub fn save_state(&self, path: &str) -> Result<()> {
        Python::with_gil(|py| {
            ...
            // Get state dict with proper PyO3 0.24 pattern
            let state_dict = model.getattr(py, "state_dict")?.call0(py)?;
            
            // Save with proper PyO3 0.24 pattern
            let save_args = PyTuple::new(py, &[(state_dict.clone_ref(py), path)]);
    ...
    pub fn load_state(&self, path: &str) -> Result<()> {
        Python::with_gil(|py| {
            ...
            // Load with proper PyO3 0.24 pattern
            let load_args = PyTuple::new(py, &[(path,)]);
            let state_dict = torch.call_method1("load", load_args?)?;
            
            let model_guard = self.model.blocking_read();
            let model = model_guard.clone_ref(py);
            
            // Load state dict with proper PyO3 0.24 pattern
...
impl ExperienceBuffer {
    ...
    fn add(&mut self, state: Vec<f32>, action: Vec<f32>, reward: f32, next_state: Vec<f32>, done: bool) {
        // Add to buffer, replacing oldest if full
        if self.experiences.len() >= self.capacity {
            self.experiences.remove(0);
        }
        
        self.experiences.push((state, action, reward, next_state, done));
    }
    
    /// Sample a batch of experiences - note this is simplified from the original implementation
    fn sample_batch(&self, batch_size: usize) -> Result<Vec<(Vec<f32>, Vec<f32>, f32, Vec<f32>, bool)>> {
...
impl NeuralNetwork for NeuralBase {
    fn forward(&self, input: &[f32]) -> Vec<f32> {
        Python::with_gil(|py| {
            ...
            let array = input.to_pyarray(py);
            
            // Get model and call forward with proper PyO3 0.24 pattern
            let model = model_guard.clone_ref(py);
            
            // Call forward with proper pattern
            let result = model.call_method1(py, "forward", (array,)).expect("Failed to call forward");
    ...
    fn train(&mut self, data: &[(Vec<f32>, Vec<f32>)]) -> anyhow::Result<()> {
        for (input, target) in data {
            Python::with_gil(|py| {
                ...
                // Convert to PyArrays
                let x = input.to_pyarray(py);
                let y = target.to_pyarray(py);
...
impl Initialize for NeuralBase {
    fn initialize(&self, config: &NeuralConfig) -> Result<()> {
        Python::with_gil(|py| {
            ...
            let _module = py.import("adaptive_network")?;
