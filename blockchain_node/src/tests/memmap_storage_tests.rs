...
mod tests {
    ...
    use crate::storage::{Storage, Hash, MemMapOptions, CompressionAlgorithm, StorageInit};
    ...
    fn test_memmap_storage_throughput() {
        ...
        for &size in &data_sizes {
            ...
            let hashes = rt.block_on(async {
                ...
                for chunk in data.chunks(100) {
                    ...
                    for item in batch.iter() {
                        let hash = memmap_storage.store(item).await.expect("Failed to store data");
                        hashes.push(hash);
                    }
                }
            ...
            let write_throughput = (num_operations * size) as f64 / write_time.as_secs_f64() / (1024.0 * 1024.0);
            ...
            rt.block_on(async {
                for hash in &hashes {
                    let retrieved = memmap_storage.retrieve(hash).await.expect("Failed to retrieve data");
                    assert!(retrieved.is_some(), "Data not found for hash");
                }
            });
            
            let read_time = start.elapsed();
            let read_throughput = (num_operations * size) as f64 / read_time.as_secs_f64() / (1024.0 * 1024.0);
            ...
            assert!(write_throughput > 50.0, "Write throughput below minimum requirement: {:.2} MB/s", write_throughput);
            assert!(read_throughput > 100.0, "Read throughput below minimum requirement: {:.2} MB/s", read_throughput);
        }
        
        // Close storage
        rt.block_on(async {
            memmap_storage.close().await.expect("Failed to close storage");
        });
    }
} 