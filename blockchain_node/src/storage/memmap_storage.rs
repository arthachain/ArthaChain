...
use crate::storage::{Storage, StorageInit, Result as StorageResult, StorageError, Hash, MemMapOptions, CompressionAlgorithm};
...
impl MemMapStorage {
    ...
    pub fn new(options: MemMapOptions) -> Self {
        Self {
            ...
            pending_batch: Arc::new(Mutex::new(Batch { writes: Vec::new(), deletes: Vec::new() })),
    ...
    fn compress_data(&self, data: &[u8]) -> (Vec<u8>, u8) {
        ...
        let result = match self.options.compression_algorithm {
            ...
            CompressionAlgorithm::Zstd => {
                match encode_all(data, 3) { // Level 3 compression (balance of speed/ratio)
            ...
            CompressionAlgorithm::Adaptive => {
                ...
                if lz4.len() <= zstd.len() && lz4.len() <= compressed.len() && lz4.len() < data.len() {
                    (lz4, 1) // LZ4
    ...
    fn decompress_data(&self, data: &[u8], algorithm: u8) -> StorageResult<Vec<u8>> {
        ...
        let result = match algorithm {
            ...
            1 => decompress_size_prepended(data) // LZ4
                .map_err(|e| StorageError::InvalidData(format!("LZ4 decompression error: {}", e))),
            2 => decode_all(data) // Zstd
                .map_err(|e| StorageError::InvalidData(format!("Zstd decompression error: {}", e))),
            3 => { // Brotli
                let mut decompressor = Decompressor::new(data, 4096);
                let mut decompressed = Vec::new();
                match decompressor.read_to_end(&mut decompressed) {
                    Ok(_) => Ok(decompressed),
                    Err(e) => Err(StorageError::InvalidData(format!("Brotli decompression error: {}", e))),
                }
            },
            _ => Err(StorageError::InvalidData(format!("Unknown compression algorithm: {}", algorithm))),
        };
    ...
    async fn flush_pending(&self) -> StorageResult<()> {
        let mut batch = {
            ...
            std::mem::replace(&mut *batch_guard, Batch { writes: Vec::new(), deletes: Vec::new() })
        };
        ...
        if !batch.writes.is_empty() {
            ...
            task::spawn_blocking(move || {
                ...
                if let (Some(ref mut data_mmap), Some(ref mut index_mmap)) = 
                    ...
                    for (hash, data) in batch.writes {
                        ...
                        if compressed_data.len() <= MAX_INLINE_DATA_SIZE {
                            ...
                            if index_offset as usize + std::mem::size_of::<IndexEntry>() + compressed_data.len() > index_mmap.len() {
                                // Grow to new size
                                let new_size = index_mmap.metadata().map_err(|e|
                                    StorageError::OperationError(format!("Failed to get current metadata: {}", e)))?
                                    .len() + BLOCK_SIZE as u64;
                                
                                index_mmap.set_len(new_size).map_err(|e|
                                    StorageError::OperationError(format!("Failed to extend index file: {}", e)))?;
                                
                                // Re-map the file
                                drop(index_map_guard); // Release lock to resize
                                
                                let new_mmap = unsafe { MmapOptions::new().map_mut(&index_mmap) }
                                    .map_err(|e| StorageError::OperationError(format!("Failed to remap index file: {}", e)))?;
                                
                                index_map_guard = index_map.write();
                                *index_map_guard = Some(new_mmap);
                                
                                // Re-check if we have enough space now
                                let total_size = std::mem::size_of::<IndexEntry>() + compressed_data.len();
                                
                                if let Some(ref mut index_mmap) = *index_map_guard {
                                    if index_offset + total_size > index_mmap.len() {
                                        return Err(StorageError::OperationError("Failed to extend index file enough".to_string()));
                                    }
                                } else {
                                    return Err(StorageError::OperationError("Index map is None after resize".to_string()));
                                }
                            }
                            ...
                            if let Some(ref mut index_mmap) = *index_map_guard {
                                ...
                                index.insert(hash, (index_offset, compressed_data.len() as u32, 0xFF)); // 0xFF = inline marker
                            }
                        } else {
                            // Append to data file
                            let offset = *data_size_guard;
                            let padded_size = (compressed_data.len() + BLOCK_SIZE - 1) / BLOCK_SIZE * BLOCK_SIZE;
                            
                            // Ensure data file has enough space
                            if (offset as usize + padded_size) > data_mmap.len() {
                                // Need to grow data file
                                drop(data_map_guard); // Release lock to resize
                                if let Some(data_file) = this.data_file.lock().as_mut() {
                                    let new_size = offset as usize + padded_size + (1024 * 1024); // Add 1MB extra
                                    data_file.set_len(new_size as u64).map_err(|e| 
                                        StorageError::OperationError(format!("Failed to resize data file: {}", e)))?;
                                    
                                    // Recreate mmap with new size
                                    let new_mmap = unsafe { MmapOptions::new().map_mut(data_file) }
                                        .map_err(|e| StorageError::OperationError(format!("Failed to remap data: {}", e)))?;
                                    
                                    data_map_guard = data_map.write();
                                    *data_map_guard = Some(new_mmap);
                                } else {
                                    return Err(StorageError::OperationError("Data file not open".to_string()));
                                }
                            }
                            ...
                            if let Some(ref mut data_mmap) = *data_map_guard {
                                ...
                                // Resize index if needed
                                if index_offset as usize + std::mem::size_of::<IndexEntry>() > index_mmap.len() {
                                    // Grow to new size
                                    let new_size = index_mmap.metadata().map_err(|e|
                                        StorageError::OperationError(format!("Failed to get current metadata: {}", e)))?
                                        .len() + BLOCK_SIZE as u64;
                                ...
                                if let Some(ref mut index_mmap) = *index_map_guard {
                                    ...
                                    // Update in-memory index
                                    index.insert(hash, (offset, compressed_data.len() as u32, compression_algo));
                    ...
                    data_mmap.flush().map_err(|e| 
                        StorageError::OperationError(format!("Failed to flush data file: {}", e)))?;
                    
                    index_mmap.flush().map_err(|e| 
                        StorageError::OperationError(format!("Failed to flush index file: {}", e)))?;
                ...
                StorageError::OperationError(format!("Task join error: {}", e)))?
        }
...
impl Storage for MemMapStorage {
    async fn store(&self, data: &[u8]) -> StorageResult<Hash> {
        ...
        let hash = Hash::new(hash_value.as_bytes());
    ...
    async fn verify(&self, hash: &Hash, data: &[u8]) -> StorageResult<bool> {
        ...
        let calculated = Hash::new(calculated_hash.as_bytes());
        Ok(calculated == *hash)
    }
...
impl StorageInit for MemMapStorage {
    async fn init(&mut self, path: Box<dyn AsRef<Path> + Send + Sync>) -> StorageResult<()> {
        ...
        std::fs::create_dir_all(base_path).map_err(|e|
            StorageError::InitError(format!("Failed to create directory: {}", e)))?;
        ...
        let index_metadata = index_file.metadata().map_err(|e|
            StorageError::InitError(format!("Failed to get index metadata: {}", e)))?;
        ...
        // Get file size and initialize if new
        let data_metadata = data_file.metadata().map_err(|e|
            StorageError::InitError(format!("Failed to get data metadata: {}", e)))?;
        ...
        if !is_new_index {
            ...
            if magic != MAGIC_BYTES {
                return Err(StorageError::InitError("Invalid magic bytes in index file".to_string()));
            }
        ...
        while offset + entry_size <= index_mmap.len() {
            ...
            if entry.hash.iter().any(|&b| b != 0) {
                ...
                if is_inline {
                    ...
                    self.index.insert(hash, (entry.offset, entry.size, entry.compression));
                }
            }
