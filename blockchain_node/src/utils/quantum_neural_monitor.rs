use crate::ai_engine::performance_monitor::{PerformanceAnomaly, PerformanceSnapshot};
...
impl QuantumNeuralMonitor {
    ...
    pub fn new(model_path: &str) -> Result<Self> {
        ...
        let window_size = model_config.window_size;
        Ok(Self {
            model_config,
            thresholds,
            data_window: Vec::with_capacity(window_size),
    ...
    pub fn predict(&mut self) -> Result<HashMap<String, f32>> {
        ...
        predictions.insert(
            "predicted_memory_usage_1h".to_string(),
            (latest.memory.physical_used_mb as f32 / latest.memory.physical_total_mb as f32)
                * 100.0
        ...
        let cpu_risk = if latest.cpu.usage_percent > self.thresholds.cpu_usage * 0.8 {
            (latest.cpu.usage_percent - self.thresholds.cpu_usage * 0.8)
                / (self.thresholds.cpu_usage * 0.2)
        } else {
            0.0
        };

        let memory_risk = if (latest.memory.physical_used_mb as f32
            / latest.memory.physical_total_mb as f32)
            * 100.0
            > self.thresholds.memory_usage * 0.8
        {
            ((latest.memory.physical_used_mb as f32 / latest.memory.physical_total_mb as f32)
                * 100.0
                - self.thresholds.memory_usage * 0.8)
                / (self.thresholds.memory_usage * 0.2)
        } else {
            0.0
        };

        let network_risk = if latest.network.latency_ms > self.thresholds.network_latency * 0.8 {
            (latest.network.latency_ms - self.thresholds.network_latency * 0.8)
                / (self.thresholds.network_latency * 0.2)
        } else {
            0.0
        };
    ...
    pub async fn detect_anomalies(
        ...
        // Check memory usage
        let memory_percent = (snapshot.memory.physical_used_mb as f32
            / snapshot.memory.physical_total_mb as f32)
            * 100.0;
            ...
            if let Some(quantum_usage) = gpu.quantum_sim_usage {
                if quantum_usage > 80.0 {
                    new_anomalies.push(self.create_anomaly(
                        ...
                        "Quantum",
                        format!(
                            "Quantum simulation resource usage is high: {}%",
                            quantum_usage
                        ),
                        vec![
        ...
        if snapshot.network.latency_ms > self.thresholds.network_latency {
            new_anomalies.push(self.create_anomaly(
                ...
                "Network",
                format!(
                    "Network latency exceeds threshold: {} ms",
        ...
        if snapshot.disk.read_ops_per_sec + snapshot.disk.write_ops_per_sec > self.thresholds.io_ops
        {
            new_anomalies.push(self.create_anomaly(
                ...
                "Disk",
                format!(
                    "I/O operations exceed threshold: {:.1} ops/sec",
        ...
        for model in &snapshot.model_inference {
            if model.avg_inference_ms > self.thresholds.model_latency {
                new_anomalies.push(self.create_anomaly(
                    ...
                    "AI Model",
                    format!(
                        "Model {} inference latency exceeds threshold: {} ms",
        ...
        if let Some(quantum) = &snapshot.quantum {
            if quantum.security_level_bits < self.thresholds.quantum_security {
                new_anomalies.push(self.create_anomaly(
                    ...
                    "Quantum Security",
                    format!(
                        "Quantum security level below threshold: {} bits",
            ...
            if quantum.pq_crypto_overhead > 20.0 {
                new_anomalies.push(self.create_anomaly(
                    ...
                    "Quantum Crypto",
                    format!(
                        "Post-quantum cryptography overhead is high: {}%",
        ...
        if let Some(anomaly_prob) = self.predictions.get("anomaly_probability_1h") {
            if *anomaly_prob > 0.7 {
                new_anomalies.push(self.create_anomaly(
                    ...
                    "Neural Prediction",
                    format!(
                        "Neural model predicts performance degradation with probability: {:.1}%",
                        anomaly_prob * 100.0
                    ),
                    vec![
        ...
        if let Some(quantum_risk) = self.predictions.get("quantum_attack_risk") {
            if *quantum_risk > 0.05 {
                new_anomalies.push(self.create_anomaly(
                    ...
                    "Quantum Security",
                    format!(
                        "Elevated risk of quantum attack: {:.2}%",
                        quantum_risk * 100.0
                    ),
                    vec![
    ...
    fn create_anomaly(
        ...
        match component {
            ...
            "Memory" => {
                related_metrics.insert(
                    "memory_used_mb".to_string(),
                    snapshot.memory.physical_used_mb as f32,
                );
                related_metrics.insert(
            ...
            "GPU" => {
                if let Some(gpu) = &snapshot.gpu {
                    ...
                    related_metrics
                        .insert("gpu_memory_used_mb".to_string(), gpu.memory_used_mb as f32);
            ...
            "Network" => {
                ...
                related_metrics.insert(
                    ...
                    snapshot.network.packet_loss_percent,
                );
                related_metrics.insert(
                    "connections".to_string(),
                    snapshot.network.p2p_connections as f32,
                );
            }
            "Disk" => {
                related_metrics.insert(
                    "read_ops_per_sec".to_string(),
                    snapshot.disk.read_ops_per_sec,
                );
                related_metrics.insert(
                    "write_ops_per_sec".to_string(),
                    snapshot.disk.write_ops_per_sec,
                );
            ...
            "Quantum Security" => {
                if let Some(quantum) = &snapshot.quantum {
                    related_metrics.insert(
                        "security_level_bits".to_string(),
                        quantum.security_level_bits as f32,
                    );
                    related_metrics
                        .insert("pq_crypto_overhead".to_string(), quantum.pq_crypto_overhead);
                    related_metrics
                        .insert("merkle_ops_per_sec".to_string(), quantum.merkle_ops_per_sec);
                }
            }
            "Quantum Crypto" => {
                if let Some(quantum) = &snapshot.quantum {
            ...
            "AI Model" => {
                ...
                if let Some(model) = snapshot.model_inference.iter().max_by(|a, b| {
                    a.avg_inference_ms
                        .partial_cmp(&b.avg_inference_ms)
                    ...
                    related_metrics
                        .insert("throughput_per_sec".to_string(), model.throughput_per_sec);
                    related_metrics.insert(
                        "accelerator_util_percent".to_string(),
