...
impl ContractFuzzer {
    ...
    where 
        F: Fn(&FuzzFunction, &HashMap<String, FuzzValue>) -> anyhow::Result<()> + Send + Sync + 'static
    {
        info!(
        ...
        for function in &self.functions {
            ...
            let handle = tokio::spawn(async move {
                for _ in 0..iterations {
                    ...
                    let result = match time::timeout(Duration::from_secs(5), async { executor_result }).await {
                        Ok(Ok(_)) => FuzzResult {
                ...
                if !seen_errors.contains_key(&error_key) {
                    ...
                    if result.unique_error {
                        ...
                        error!(
                            ...
                            result.error_message.as_ref().unwrap_or(&"No message".to_string())
                        );
                    }
                }
            }
    ...
    fn save_results(&self, path: &str) -> anyhow::Result<()> {
        ...
        std::fs::write(path, json)
            .context("Failed to write fuzzing results to file")?;
    ...
    pub fn get_summary(&self) -> String {
        ...
        let unique_failures = self.results.iter().filter(|r| !r.success && r.unique_error).count();
        ...
        if unique_failures > 0 {
            ...
            for fuzz_result in self.results.iter().filter(|r| !r.success && r.unique_error) {
                result.push_str(&format!(
                    ...
                    fuzz_result.error_message.as_ref().unwrap_or(&"N/A".to_string())
                ));
            }
    ...
    fn classify_error(error_message: &str) -> FuzzErrorType {
        ...
        } else if message.contains("gas") || message.contains("memory limit") || message.contains("resource") {
            FuzzErrorType::ResourceExhaustion
        } else if message.contains("memory") || message.contains("corrupt") {
            FuzzErrorType::MemoryCorruption
        } else if message.contains("overflow") || message.contains("underflow") || message.contains("arithmetic") {
            FuzzErrorType::IntegerOverflow
        } else if message.contains("unauthorized") || message.contains("permission") || message.contains("access denied") {
...
pub async fn fuzz_wasm_executor() -> anyhow::Result<()> {
    ...
    let executor = |function: &FuzzFunction, inputs: &HashMap<String, FuzzValue>| -> anyhow::Result<()> {
