use prometheus::{Registry, Counter, IntGauge, Histogram, HistogramOpts};
...
impl MetricsCollector {
    ...
    pub fn new(_config: &Config) -> Result<Self> {
        ...
        // Create metrics
        let block_count = IntGauge::new("artha_block_count", "Total number of blocks").context("Failed to create block count gauge")?;
        let transaction_count = Counter::new("artha_transaction_count", "Total number of transactions").context("Failed to create transaction count counter")?;
        let peer_count = IntGauge::new("artha_peer_count", "Number of connected peers").context("Failed to create peer count gauge")?;
