...
pub fn ed25519_verify(public_key_bytes: &[u8], data: &[u8], signature_bytes: &[u8]) -> Result<bool> {
...
pub fn recover_address_from_signature(message: &[u8], signature: &[u8]) -> Result<crate::types::Address> {
    if signature.len() != 65 {
        return Err(anyhow!("Invalid signature length. Expected 65 bytes, got {}", signature.len()));
    }
...
pub fn sign_message(private_key: &[u8], message: &[u8]) -> Result<Vec<u8>> {
    let signing_key = SigningKey::from_bytes(private_key.try_into().map_err(|_| anyhow!("Invalid private key length"))?);
    let signature = signing_key.sign(message);
    Ok(signature.to_bytes().to_vec())
}

/// Verify address signature
pub fn verify_address_signature(address: &Address, data: &[u8], signature_bytes: &[u8]) -> Result<bool> {
    let mut key_bytes = [0u8; 32];
    key_bytes[..20].copy_from_slice(address.as_bytes());
    key_bytes[20..].fill(0);
