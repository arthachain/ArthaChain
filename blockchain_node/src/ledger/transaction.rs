...
impl Transaction {
    ...
    /// Calculate transaction hash (used as transaction ID)
    ...
    #[cfg(feature = "bls")]
    pub fn sign_bls(&mut self, bls_private_key: &threshold_crypto::SecretKey) -> Result<(), TransactionError> {
        let message = self.serialize_for_hash();
    ...
    pub fn verify_signature(&self) -> Result<bool, TransactionError> {
        ...
        let sender_pk = hex::decode(&self.sender)
    ...
    #[cfg(feature = "bls")]
    pub fn verify_bls_signature(&self, public_key: &BlsPublicKey) -> Result<bool, TransactionError> {
    ...
    pub fn validate(&self) -> Result<(), TransactionError> {
        ...
        match self.tx_type {
            ...
            TransactionType::Stake | TransactionType::Delegate => {
                if self.amount < 100 {  // Minimum stake amount
    ...
    pub fn create_batch(transactions: Vec<Transaction>, sender: String) -> Result<Self, TransactionError> {
...
mod tests {
    ...
    fn test_transaction_signing_and_verification() {
        ...
        println!("Generated keypair - private key len: {}, public key len: {}", 
    ...
    fn test_transaction_validation() {
        ...
        assert!(matches!(tx_invalid.validate(), Err(TransactionError::InvalidSender)));
        
        // Empty recipient
        let mut tx_invalid = tx.clone();
        tx_invalid.recipient = "".to_string();
        assert!(matches!(tx_invalid.validate(), Err(TransactionError::InvalidRecipient)));
        ...
        assert!(matches!(tx_invalid.validate(), Err(TransactionError::InvalidSignature)));
    }
