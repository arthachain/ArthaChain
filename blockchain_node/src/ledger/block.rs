...
use crate::ledger::{ConsensusError, TransactionError, BlockValidationError};
...
impl ConsensusInfo {
    ...
    pub fn add_signature(
        ...
        if self.validator_signatures.iter().any(|s| s.validator_id == validator_id) {
            return false;
        }
    ...
    pub fn add_cross_shard_ref(&mut self, shard_id: u64, block_hash: Hash, height: u64) {
        ...
        if self.cross_shard_refs.iter().any(|r| r.shard_id == shard_id && r.height == height) {
            return;
        }
    ...
    pub fn verify_aggregate_bls_signature(&self, message: &[u8]) -> Result<bool, ConsensusError> {
        ...
        for sig in &self.validator_signatures {
            if sig.signature_type == SignatureType::BLS {
                ...
                // Parse public key
                let pk_bytes = hex::decode(&sig.validator_id)
                    .map_err(|_| ConsensusError::InvalidSignature)?;
...
impl Block {
    ...
    pub fn sign_as_validator(&mut self, validator_id: String, signature: Vec<u8>) -> bool {
        self.consensus.add_signature(validator_id, signature, SignatureType::Ed25519)
    }
    
    /// Sign the block using BLS signature
    #[cfg(feature = "bls")]
    pub fn sign_as_validator_bls(&mut self, validator_id: String, signature: Vec<u8>) -> bool {
        self.consensus.add_signature(validator_id, signature, SignatureType::BLS)
    }
    
    /// Change the consensus status
    pub fn update_consensus_status(&mut self, new_status: ConsensusStatus) -> Result<(), ConsensusError> {
    ...
    pub fn validate_transactions(&self, state: &BlockchainState) -> Result<(), TransactionError> {
        ...
        for tx in &self.body.transactions {
            ...
            let sender_nonces = used_nonces.entry(tx.sender.clone()).or_insert_with(HashSet::new);
    ...
    pub async fn validate_transactions_parallel(&self, state: Arc<BlockchainState>) -> Result<(), TransactionError> {
        use tokio::task;
        
        // First validate each transaction independently in parallel
        let validation_tasks: Vec<_> = self.body.transactions
            .iter()
            .map(|tx| {
        ...
        for task in validation_tasks {
            task.await.map_err(|e| TransactionError::Internal(e.to_string()))?
                      .map_err(|e| e)?;
        }
    ...
    pub fn validate(&self, state: &BlockchainState) -> Result<(), BlockValidationError> {
        ...
        if calculated_hash != self.header.hash {
            return Err(BlockValidationError::Other("Invalid block hash".to_string()));
        }

        // Validate transactions
        self.validate_transactions(state).map_err(|e| BlockValidationError::InvalidTransactions(e))?;
