...
use crate::evm::types::{EvmAddress, EvmTransaction, EvmExecutionResult, EvmConfig};
use ethereum_types::{H160, H256, U256};
use jsonrpc_core::{IoHandler, Value, Params, Error as RpcError, ErrorCode};
use jsonrpc_http_server::{ServerBuilder, Server as RpcServer};
...
impl EvmRpcService {
    ...
    pub fn start(&mut self, addr: SocketAddr) -> Result<(), anyhow::Error> {
        ...
        io.add_method("eth_getBalance", move |params: Params| {
            // Parse parameters
            let params: (String, String) = params.parse().map_err(|e| {
            ...
            let address = if address_str.starts_with("0x") {
                let address_bytes = hex::decode(&address_str[2..]).map_err(|e| {
                    RpcError::invalid_params(format!("Invalid address: {:?}", e))
                })?;
        ...
        io.add_method("eth_estimateGas", move |params: Params| async move {
            ...
            let tx = EvmTransaction {
                ...
                gas_price: call_request.gas_price.unwrap_or(U256::from(executor_clone.get_config().default_gas_price)),
        ...
        io.add_method("eth_call", move |params: Params| async move {
            ...
            let tx = EvmTransaction {
                ...
                data: call_request.data.unwrap_or_else(Vec::new),
                gas_price: call_request.gas_price.unwrap_or(U256::from(executor_clone.get_config().default_gas_price)),
                gas_limit: call_request.gas.unwrap_or(U256::from(executor_clone.get_config().default_gas_limit)),
