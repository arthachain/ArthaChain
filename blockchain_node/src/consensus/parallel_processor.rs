...
impl ParallelProcessor {
    ...
    pub fn calculate_block_parameters(&self) -> BlockParameters {
        ...
        info!("Dynamic parameters: block_time={:.2}s, batch_size={} with {} miners", 
    ...
    pub async fn start(&self) -> Result<JoinHandle<()>> {
        ...
        let handle = tokio::spawn(async move {
            ...
            while running.load(Ordering::Relaxed) == 1 {
                ...
                let _block_time = (base_block_time / (current_miners as f32 * throughput_multiplier).max(1.0)).max(0.5);
                let batch_size = ((base_batch_size as f32) * (current_miners as f32 * throughput_multiplier).max(1.0)) as usize;
                ...
                let pending_txs: Vec<crate::ledger::transaction::Transaction> = pending_txs.into_iter().map(Into::into).collect();
                ...
                match Self::mine_concurrent_blocks(
                    &segments,
                    &state,
                ).await {
                    Ok(blocks) => {
    ...
    async fn mine_segment_block(
        ...
        debug!("Mined segment {} block with {} transactions", segment_id, block.body.transactions.len());
    ...
    async fn merge_parallel_blocks(blocks: Vec<Block>) -> Result<Block> {
        ...
        let combined_nonce = blocks.iter().fold(0u64, |acc, block| acc ^ block.header.nonce);
        merged_block.set_nonce(combined_nonce);
        
        info!("Merged {} blocks with total {} transactions", 
              blocks.len(), merged_block.body.transactions.len());
...
mod tests {
    ...
    async fn test_dynamic_parameters() {
        ...
        println!("With 1 miner: block_time={}, batch_size={}", params.block_time, params.batch_size);
        ...
        let params = processor.calculate_block_parameters();
        println!("With 4 miners: block_time={}, batch_size={}", params.block_time, params.batch_size);
        ...
        assert!(params.block_time <= 15.0, "Block time should not exceed base block time");
        assert!(params.batch_size >= 500, "Batch size should not be less than base batch size");
    }
} 