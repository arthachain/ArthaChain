use crate::consensus::cross_shard::{
    CoordinatorMessage, CrossShardConfig, CrossShardCoordinator, CrossShardManager,
    CrossShardTransaction, ParticipantHandler, TxPhase,
};
...
pub struct EnhancedCrossShardManager {
    ...
    #[allow(dead_code)]
    quantum_key: Vec<u8>,
    /// Message channels - only store sender since receiver can't be cloned
    #[allow(dead_code)]
...
impl EnhancedCrossShardManager {
    ...
    pub async fn new<T: 'static + Send + Sync>(
        ...
        let (_public_key, private_key) = generate_quantum_resistant_keypair(None)?;
        ...
        let participant =
            ParticipantHandler::new(config.clone(), private_key.clone(), participant_sender);
    ...
    pub fn start(&mut self) -> Result<()> {
        ...
        self.manager
            .start()
            .map_err(|e| anyhow!("Manager start error: {}", e))?;
    ...
    pub async fn initiate_cross_shard_transaction(
        ...
        let tx_id = format!("tx_{}", transaction.tx_hash);
        ...
        let tx_data = transaction.tx_hash.as_bytes().to_vec();
        ...
        let cross_shard_msg = crate::network::cross_shard::CrossShardMessage {
            ...
            payload: transaction.tx_hash.as_bytes().to_vec(),
        ...
        self.manager
            .add_transaction(cross_shard_msg)
            .map_err(|e| anyhow!("Add transaction error: {}", e))?;

        info!(
            "Initiated cross-shard transaction {} with quantum-resistant 2PC",
            tx_id
        );
    ...
    pub fn get_transaction_status(&self, tx_id: &str) -> Option<(TxPhase, bool)> {
        ...
        self.manager
            ...
            .map(|status| match status {
                ...
                TransactionStatus::Success => (TxPhase::Commit, true),
                TransactionStatus::Failed(_) => (TxPhase::Abort, true),
                TransactionStatus::Expired => (TxPhase::Abort, true),
            })
    }
