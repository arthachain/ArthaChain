...
impl SVBFTConsensus {
    ...
    pub async fn start(&mut self) -> Result<JoinHandle<()>> {
        ...
        let mut message_receiver = std::mem::replace(&mut self.message_receiver, mpsc::channel::<ConsensusMessage>(100).1);
        let message_sender = self.message_sender.clone();
        let mut block_receiver = std::mem::replace(&mut self.block_receiver, mpsc::channel::<Block>(100).1);
        ...
        let handle = tokio::spawn(async move {
            ...
            let quorum_size = Self::calculate_quorum_size(&validators, &node_capabilities, &svbft_config).await;
            ...
            let leader = Self::select_leader_for_view(0, &validators_copy).unwrap_or_else(|| node_id.clone());
            ...
            info!("SVBFT consensus initialized with quorum size: {}", quorum_size);
    ...
    async fn initialize_validators(
        ...
        for i in 1..=10 {
            ...
            let latency_ms = if is_mobile { 100 + (i * 10) } else { 50 + (i * 5) };
            let bandwidth_mbps = if is_mobile { 10 } else { 100 };
            
            capabilities_map.insert(node_id, NodeCapabilities {
    ...
    async fn calculate_quorum_size(
        ...
        let mobile_count = capabilities.values()
            .filter(|cap| cap.is_mobile)
            .count();
        
        let low_bandwidth_count = capabilities.values()
    ...
    async fn handle_new_block(
        ...
        if round.leader == node_id {
            ...
            debug!("Received block but not the leader for view {}, ignoring", view);
        }
    ...
    async fn handle_proposal(
        ...
        // Add own vote
        if let ConsensusMessage::Prepare { node_id, signature, .. } = prepare {
            round.prepare_votes.insert(node_id, signature);
        }
        
        info!("Received proposal for block {} in view {}, sent prepare vote", block.hash(), view);
    ...
    async fn handle_prepare(
        ...
        if round.has_quorum_for_phase(ConsensusPhase::Prepare) {
            ...
            // Add own vote
            if let ConsensusMessage::PreCommit { node_id, signature, .. } = precommit {
                round.precommit_votes.insert(node_id, signature);
            }
            
            info!("Prepare quorum reached for block {} in view {}, sent pre-commit vote", 
                hex::encode(&block_hash), view);
        }
    ...
    async fn handle_precommit(
        ...
        if round.has_quorum_for_phase(ConsensusPhase::PreCommit) {
            ...
            // Add own vote
            if let ConsensusMessage::Commit { node_id, signature, .. } = commit {
                round.commit_votes.insert(node_id, signature);
            }
            
            info!("Pre-commit quorum reached for block {} in view {}, sent commit vote", 
                hex::encode(&block_hash), view);
        }
    ...
    async fn handle_commit(
        ...
        if round.has_quorum_for_phase(ConsensusPhase::Commit) {
            ...
            // Add own vote
            if let ConsensusMessage::Decide { node_id, signature, .. } = decide {
                round.decide_votes.insert(node_id, signature);
            }
            
            info!("Commit quorum reached for block {} in view {}, sent decide vote", 
                hex::encode(&block_hash), view);
        }
    ...
    async fn handle_consensus_message(
        ...
        match message {
            ConsensusMessage::Proposal { view: msg_view, block, node_id: proposer, signature } => {
                // Ignore messages for different views
                if msg_view != view {
                    debug!("Ignoring proposal for different view: {} (current: {})", msg_view, view);
                    return Ok(());
                }
                
                Self::handle_proposal(
                    view, block, proposer, signature, round, node_id, message_sender
                ).await?;
            },
            
            ConsensusMessage::Prepare { view: msg_view, block_hash, node_id: voter, signature } => {
                // Ignore messages for different views
                if msg_view != view {
                    debug!("Ignoring prepare for different view: {} (current: {})", msg_view, view);
                    return Ok(());
                }
                
                Self::handle_prepare(
                    view, block_hash, voter, signature, round, node_id, message_sender, &validators_set
                ).await?;
            },
            
            ConsensusMessage::PreCommit { view: msg_view, block_hash, node_id: voter, signature } => {
                // Ignore messages for different views
                if msg_view != view {
                    debug!("Ignoring pre-commit for different view: {} (current: {})", msg_view, view);
                    return Ok(());
                }
            ...
            ConsensusMessage::Commit { view: msg_view, block_hash, node_id: voter, signature } => {
                // Ignore messages for different views
                if msg_view != view {
                    debug!("Ignoring commit for different view: {} (current: {})", msg_view, view);
                    return Ok(());
                }
            ...
            ConsensusMessage::Decide { view: msg_view, block_hash, node_id: voter, signature } => {
                // Ignore messages for different views
                if msg_view != view {
                    debug!("Ignoring decide for different view: {} (current: {})", msg_view, view);
                    return Ok(());
                }
                ...
                if round.has_quorum_for_phase(ConsensusPhase::Decide) {
                    ...
                    if let Some(block) = &round.proposed_block {
                        ...
                        info!("Decide quorum reached for block {} in view {}, block finalized", 
                            hex::encode(&block_hash), view);
            ...
            ConsensusMessage::NewView { new_view, node_id: sender, signatures, new_block } => {
                ...
                if new_view <= view {
                    debug!("Ignoring new view {}, not higher than current view {}", new_view, view);
                    return Ok(());
                }
                ...
                // Start a new round
                let quorum_size = Self::calculate_quorum_size(validators, node_capabilities, svbft_config).await;
                let leader = Self::select_leader_for_view(new_view, &validators_set).unwrap_or_else(|| node_id.to_string());
                let base_timeout = Duration::from_millis(svbft_config.base_timeout_ms);
                
                {
                    let mut round_guard = current_round.lock().await;
                    *round_guard = Some(ConsensusRound::new(new_view, leader.clone(), quorum_size, base_timeout));
    ...
    async fn advance_to_next_view(
        ...
        let leader = {
            let validators_set = validators.read().await;
            Self::select_leader_for_view(new_view, &validators_set).unwrap_or_else(|| node_id.to_string())
        };
...
mod tests {
    ...
    async fn test_quorum_size_calculation() {
        ...
        let quorum_size_no_adaptive = SVBFTConsensus::calculate_quorum_size(
            &validators, &node_capabilities, &config_no_adaptive
        ).await;
        ...
        let quorum_size_adaptive = SVBFTConsensus::calculate_quorum_size(
            &validators, &node_capabilities, &config_adaptive
        ).await;
        ...
        let quorum_size_fixed = SVBFTConsensus::calculate_quorum_size(
            &validators, &node_capabilities, &config_fixed
        ).await;
