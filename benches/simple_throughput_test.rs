...
fn main() {
    ...
    for &tx_size in &tx_sizes {
        for &tx_count in &tx_counts {
            println!("\nTesting with {} transactions of {} bytes each:", tx_count, tx_size);
            ...
            for &thread_count in &thread_counts {
                ...
                if thread_count == 1 {
                    ...
                    println!("  Single-threaded: {:.2} TPS ({} ms)", 
                             tps, duration.as_millis());
                }
                ...
                println!("  {} threads: {:.2} TPS ({} ms)", 
                         thread_count, tps, duration.as_millis());
            }
        }
    }
}
...
fn process_transactions_parallel(transactions: &[Vec<u8>], threads: usize) -> usize {
    let chunk_size = (transactions.len() + threads - 1) / threads;
    ...
    for chunk in chunks {
        ...
        let chunk_data: Vec<Vec<u8>> = chunk.iter().map(|tx| tx.clone()).collect();
